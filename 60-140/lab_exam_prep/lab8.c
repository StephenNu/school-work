...
/*
	To change the number of columns you would only need to change numberOfTests by that number, and then changes [6] to be 
	2 more then whatever number is in numberOfTests.
	To change the number of rows ie students that can be entered, change the numberOfStudents to that number and change the 
	first declartion of numTable from [12] to be 2 more then whatever number is in numberOfStudents.
*/
...
/* 
	RowCalc() takes a [] by [6] array and calculates the average and minimum values for each row upto a total equal to maxRows.
*/
